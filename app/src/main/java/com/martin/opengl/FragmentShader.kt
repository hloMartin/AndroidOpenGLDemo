...
const val FRAGMENT_SHADER_DEMO1 = """
		...
		const float scaleX = 40.0;//min:0.1, max:60
		const float scaleY = 40.0;//min:0.1, max:60
...
const val FRAGMENT_SHADER_DEMO2 = """
            precision mediump float;
            varying vec2 v_TexCoord;
            uniform sampler2D u_TextureUnit;
            uniform sampler2D u_TextureUnit1;
            ...
            void main(){
                float x = smoothstep(.0, 1.0, (progress*2.0+v_TexCoord.y-1.0));
			    gl_FragColor = mix(
			        texture2D(u_TextureUnit, (v_TexCoord -.5)*(1.-x)+.5),
			        texture2D(u_TextureUnit1,(v_TexCoord-.5)*x+.5),
				    x);
            }
"""
...
fun getDemoFragmentShader(index: Int): String {
    return when (index) {
        1 -> {
            image01 = R.drawable.img11
            image02 = R.drawable.img12
            FRAGMENT_SHADER_DEMO1
        }
        2 -> {
